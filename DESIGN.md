...
| The timeline shows five entries at a time, with three levels of detail | The history stays scannable as dates and titles while exactly one commit is readable, which is what keeps a nineteen-entry rail from being a wall. Selection is explicit — click, arrow keys, or the pager — rather than derived from scroll position, and the expanded card is a third level asked for on purpose. The card is bounded and scrolls rather than truncating, because "show details" has to mean all of them; a card free to grow would push the other four commits off the screen and undo the window. | Exactly one entry may show a summary, and at most one may show a card. The visible "n of m" and the range readout are what tell you the rest of the history exists, so neither is optional chrome. The in-card arrows appear only when the body actually overflows, and the body is focusable so it scrolls by keyboard without them. Verify by reaching the last entry with the keyboard alone. |
