...
| Each project page has an "Explore releases in 3D" button that loads `three` | Asked for directly, and bounded the way R29 requires: it never touches the initial page — `src/releaseExplorer.ts` and its scene builders (`src/releaseScenes.ts`) are reached only through `import()` inside the button's click handler, the exact discipline `commitBody.js` already established for KaTeX. The resulting chunk is ~719 KB minified (esbuild's code-splitting doesn't fully tree-shake `three`'s single pre-bundled `three.module.js` across a dynamic-import boundary even with named imports) — comparable to, not larger than, the already-accepted 647 KB KaTeX chunk, and likewise never fetched by a visitor who doesn't click the button. The explorer is strictly additive: `mountTimelineWindow`'s pager (the primary, fully accessible view) is untouched on every page. | `js/site.js` must not grow meaningfully when this feature is added (checked: 69→75 KB) and no request for the `three`/`releaseScenes` chunks may fire before the trigger button is clicked. If a future change makes the `three` chunk meaningfully larger without new capability, that's worth revisiting; the number itself is not a hard ceiling. |
| The release explorer is a hand-rolled `role="dialog"`, not a `<dialog>` element or a borrowed pattern | This is the first modal in the codebase — grep for `role="dialog"`/`aria-modal`/`.modal` before this feature found nothing. Recorded here so the next full-screen overlay reuses this rather than inventing a third pattern: `aria-modal="true"` plus a hand-written focus trap (Tab/Shift+Tab cycle between the close button and the last focusable list item; Escape closes), focus moves to the close button on open and back to the trigger button on close. The overlay itself is shaped like `.guest-window.is-overlay` (fixed, `inset: 0`, its own `z-index`) rather than the real Fullscreen API — deliberately: that API's per-browser permission/gesture behaviour isn't needed for what was actually asked for (a full-viewport view with an escape hatch), and a plain fixed element is simpler to reason about and test. | Open the explorer and Tab repeatedly: focus must never leave the dialog, and Shift+Tab from the close button must land on the last list item (and Tab from the last item back on the close button). Escape and the visible `[X]` must both close it and return focus to the button that opened it — verified with Playwright on all 5 project pages. |
| Release-explorer scenes are themed by a hand-tagged `theme` field on curated events, and a keyword guess for generated ones | There is no category field on a timeline event otherwise (`kind` is only `"feature" \| "release"`). The 50 curated entries in `src/data.ts` were each read and tagged directly against what they actually say (`src/releaseThemes.ts` lists the ten theme keys). Auto-synced `generatedTimelineEvents` refresh daily with no human curation, so they cannot be hand-tagged the same way — `themeForEvent()`'s keyword fallback is an explicitly best-effort, purely decorative categorization for those, never asserted as fact: getting one wrong picks the wrong background prop, not a wrong claim on the page, since the event's real title and detail text are what's actually stated and render unchanged either way. This is a narrower, more deliberate distinction than DESIGN.md B5's no-fabrication standard technically requires, drawn because a 3D scene reads as more "asserted" than plain background decoration would. | An event's explicit `theme` (curated) must always win over the keyword guess — covered by `tests/releaseThemes.test.mjs`. A generated event with no keyword match must fall back to `core`, never a guess with no textual basis. |
