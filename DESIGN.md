...
evidence_and_known_constraints:
  ...
  - "KeyQuorum's lab is a third-party-origin iframe from bpforbes.github.io too. Its USB drives are mocks with published passphrases; the page and the lab both say it is not hardware-backed."
...
required_states:
  - "Live lab: connecting, loaded-without-handshake, attached, could-not-attach (with retry and open-in-new-tab), minimized, not-attached (kept for a future guest with no web build)"
...
out_of_scope:
  ...
  - "Building KeyQuorum's lab here — it is built and published from the KeyQuorum repository"
```
...
| KeyQuorum's live lab is a live guest, and the "not attached" panel stays in the window | KeyQuorum now publishes a web build, so its panel was replaced by a live guest with the same strict handshake as Flinstone (`schemaVersion` 1 plus the source commit). The offline panel is kept for any future project with no web build. | Delete the offline mode if no guest has used it by the next redesign. |
...
- **Live lab**: connecting → could-not-attach → retry exercised with the guest
  ...
  KeyQuorum attaches through the same states as the other guests.
