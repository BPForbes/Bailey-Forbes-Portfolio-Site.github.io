...
| The timeline shows five entries at a time, with three levels of detail | The history stays scannable as dates and titles while exactly one commit is readable, which is what keeps a nineteen-entry rail from being a wall. Selection is explicit — click, arrow keys, or the pager — rather than derived from scroll position, and the expanded card is a third level asked for on purpose. Every entry is reachable without a pointer: up/down move the selection and page across the window boundary, left/right page, Home/End jump to either end. | Exactly one entry may show a summary, and at most one may show a card. The visible "n of m" and the range readout are what tell you the rest of the history exists, so neither is optional chrome. Verify by reaching the last entry with the keyboard alone. |
...
| Commit detail is rendered as Markdown and LaTeX by a renderer written here | The detail comes from pull request bodies, so it already contains `**bold**`, backticked identifiers and `-` lists; printing it as plain text put that punctuation on the page. A Markdown parser plus a TeX engine would be more bytes than the whole site and R29 keeps third-party runtime requests off the page, so `src/richText.ts` implements a documented subset. It builds DOM nodes and never assembles HTML strings, because this is other people's writing arriving through an API. | No code path may pass commit text to `innerHTML`. Link targets are restricted to http, https and mailto — a `javascript:` or `data:` target must render as plain words. Math the subset cannot parse must fall back to its own source in a `<code>`, never to a guess. `__init__` must not become bold. Covered by `tests/richtext.test.mjs`. |
